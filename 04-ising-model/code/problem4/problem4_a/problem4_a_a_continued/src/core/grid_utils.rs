...
use crate::core::evaluate_state::{_calc_dh, calc_h_sum};
...
// Performing a sweep of the grid with n_try hits per spin at temperature beta
pub fn _sweep<const L: usize>(state: &mut [[i8; L]; L], beta: f64, n_try: usize) {
    let mut rng = rand::thread_rng();
    for row in 0..L {
        for col in 0..L {
            // Calculating the change in energy from the spin flip
            let mut dh = _calc_dh(state, (row, col));
            // Calculating the probability in the case that dh is less than 0
            let p_preemptive = (-beta * dh.abs()).exp();

            for _ in 0..n_try {
                if dh <= 0.0 {
                    // Flipping the spin if energy is decreased
                    state[row][col] *= -1;
                    dh *= -1.0; // Updating dh due to spin flip
                } else {
                    let p_rand: f64 = rng.gen_range(0.0..1.0);
                    if p_rand < p_preemptive {
                        state[row][col] *= -1;
                        dh *= -1.0; // Updating dh due to spin flip
                    }
                }
...
pub fn sweep_heatbath<const L: usize>(state: &mut [[i8; L]; L], beta: f64) {
    let mut rng = rand::thread_rng();
    for row in 0..L {
        for col in 0..L {
